...
module.exports = {
	functions: {
		'linear-gradient': function (fn) {
			...
			this.parent.add(new tree.rule(this.name, this.value.replace(fn.string, getWebkitOldSyntax(params))), index).vendor = 'webkit';
			this.parent.add(new tree.rule(this.name, this.value.replace(fn.string, '-moz-linear-gradient(' + replace + ')')), index).vendor = 'moz';
			this.parent.add(new tree.rule(this.name, this.value.replace(fn.string, '-webkit-linear-gradient(' + replace + ')')), index).vendor = 'webkit';
			this.parent.add(new tree.rule(this.name, this.value.replace(fn.string, '-o-linear-gradient(' + replace + ')')), index).vendor = 'o';
		}
	},
	enabled: true,
