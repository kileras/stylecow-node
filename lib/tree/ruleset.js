		...
		transform: function (plugins, data, support) {
			...
			for (var name in data) {
				if (data.hasOwnProperty(name)) {
					propatatedData[name] = utils.clone(data[name]);
				}
			}

			executePlugins(plugins, propatatedData, 'children', this, support);

			if (this.selector) {
				executePlugins(plugins, propatatedData, 'selector', this.selector, support);
			} else if (!this.parent) {
				executePlugins(plugins, propatatedData, 'init', this, support);
			}
			...
			this.getRules().forEach(function (rule) {
				...
				executePlugins(plugins, propatatedData, 'rule', rule, support);
			});

			this.getChildren().forEach(function (child) {
				child.transform(plugins, propatatedData, support);
			});
		},
