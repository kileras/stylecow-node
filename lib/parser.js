...
var parser = {
	...
	parseImport: function (string) {
		var matches = string.match(/^\@import (url\()?['"]?([^'"\)]+)/);
				...
				switch (currChar) {
					...
					case '{':
						switch (status[0]) {
							case 'selector':
							case 'rules':
								child = child.add(new tree.ruleset(parser.parseSelector(buffer)));
					...
					case '}':
						switch (status[0]) {
							case 'rules':
								if (buffer.trim()) {
									child.add(new tree.rule(parser.parseRule(buffer)));
								}
								buffer = '';
								status.shift();
								child = child.parent;
						...
						switch (status[0]) {
							case 'selector':
								if (buffer.indexOf('@import') === 0) {
									child.add(parser.parseImport(buffer)).setSourceColLine(col, line);
								} else {
									child.add(new tree.ruleset(parser.parseSelector(buffer))).setSourceColLine(col, line);
								}
								buffer = '';
								break;

							case 'rules':
								child.add(parser.parseRule(buffer));
								buffer = '';
								break;
						}
						break;
